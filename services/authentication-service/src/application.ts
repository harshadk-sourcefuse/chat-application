...
export class AuthenticationServiceApplication extends BootMixin(
  ...
  constructor(options: ApplicationConfig = {}) {
    ...
    options.rest.port = +(process.env.AUTH_SERVICE_PORT ?? port);
    ...
    this.bind(ServiceBindings.USER_SERVICE).toClass(UserService).inScope(BindingScope.SINGLETON);
    this.bind(ServiceBindings.TOKEN_SERVICE).toClass(TokenService).inScope(BindingScope.SINGLETON);

    this.bind(Strategies.Passport.GOOGLE_OAUTH2_VERIFIER).toProvider(GoogleOauthProvider);
    // this.bind(Strategies.Passport.BEARER_TOKEN_VERIFIER).toProvider(BearerTokenAuth);
    this.bind(SignUpBindings.GOOGLE_SIGN_UP_PROVIDER).toProvider(GoogleSignupProvider)
